...
/**
 * Select a specific select box (option box) value by setting the
 *  select box's selected index property to the option that has
 *  that value.
 *
 * @param {Object} selectBox - A Valid DOM element that is a SELECT
 *  box.
 * @param {String} valueToSelect - The value of the option box to select.
 *
 * @return {boolean} - Returns TRUE if an option with the desired value was
 *  found and selected, otherwise FALSE.
 */
function setSelectedIndexByValue(selectBox, valueToSelect) {
	const errPrefix = `(setSelectedIndex) `;

	if (!misc_shared_lib.isNonNullObjectAndNotArray(selectBox))
		throw new Error(errPrefix + `The select box parameter does not contain a valid object.`);
	if (!selectBox.tagName)
		throw new Error(errPrefix + `The tagName property was not found in the given select box.`);
	if (selectBox.tagName.toLowerCase() !== 'select')
		throw new Error(errPrefix + `The object given in the select box parameter is not a select box.  Tag name found: ${selectBox.tagName}`);

	if (misc_shared_lib.isEmptySafeString(valueToSelect))
		throw new Error(errPrefix + `The valueToSelect parameter is empty.`);

	const valueToSelectToLower = valueToSelect.toLowerCase();

	for(let i = 0; i < selectBox.options.length; i++) {
		if(selectBox.options[i].innerHTML.toLowerCase() === valueToSelectToLower) {
			selectBox.selectedIndex = i;

			// Desired option value found and selected.
			return true;
		}
	}

	// Unable to find a matching entry.
	return false;
}
...
if (typeof module == 'undefined' || typeof module.exports == 'undefined')
{
	// Browser context.
	/**
	 * This function adorns the HTMLMediaElement object with
	 *  a new property named "is_playing", which will be TRUE
	 *  if an "audio" or "video" element is playing, and FALSE
	 *  if not.
	 *
	 * Example usage (also works with jQuery selectors too):
	 *
	 *  const bIsPlaying = document.getElementById("myVideo").is_playing;
	 *
	 */
	Object.defineProperty(HTMLMediaElement.prototype, 'is_playing', {
		get: function() {
			// The logic is, if the video playback head is ahead of zero, and
			//  it is not paused, and it has not ended, and it is in a ready
			//  state that indicates a viable video is loaded, then it is
			//  playing.
	...
	window.misc_shared_lib.setSelectedIndexByValue = setSelectedIndexByValue;
}
else
{
	// Server context.
