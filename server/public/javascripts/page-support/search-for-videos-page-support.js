...
function doSearch()
{
	...
	try
	{
		var searchQuery = $('#' + g_SearchQueryInputId).val();
		...
		var jqxhr = $.post('search-youtube-api',
						   requestObj,
						   function ()
						   {
	...
	{
		var errMsg = errPrefix + 'Error executing video search -> ' + err.message;
...
$(document).ready(function (){
	...
	// Get any available URL arguments.
	let urlArgs = getUrlArguments();
	...
	// Get the app configuration from the server.  We don't wait
	//  on the promise because getMyConfig_promise() will
