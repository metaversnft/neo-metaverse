...
// The following objects are only used if we are in
// 	the server execution context.
let common_routines = null;
let youtube_search = null;
let YouTubeSimpleApiClass = null;

// Get our YouTube API key from the environment.
let tempYouTubeApiKey = null;

let youtubeSimpleApi = null;

// Don't use "misc_shared_lib" or we may wipe out the value assignee to
//  it when that module initializes in the browser.  Instead, just use
//  our own name for it.
let misc_shared_lib_yas = null;
...
{
	let errPrefix = `(getYouTubeApiKey) `;
...
function getChannelDescriptionByVideoId_promise(videoId)
{
	...
	return new Promise(function(resolve, reject) {
		...
		.then(youtubeChannel => {
			if (!youtubeChannel)
				throw new Error(errPrefix + 'Unable to find a channel with channel id: ' + channelId);
...
function extractBandPublicAddrFromChannelDesc(channelDescription) {
	...
	if (!misc_shared_lib_yas.isEmptySafeString(channelDescription))
	{
		...
		if (aryElements && (aryElements.length > 1) && !misc_shared_lib_yas.isEmptySafeString(aryElements[1]))
		{
...
const YouTubeSupport = (function() {
	...
	/** @property {Array<function>>} - Any subscribers to the player's
	 * 	timeupdate event will be stored here. */
	this.aryTimeUpdateListeners = [];

	/**
	 * This function returns TRUE if the listener function is
	 * 	currently subscribed to the player's timeupdate event.
	 * 	Otherwise it returns FALSE.
	 *
	 * @param {Function} funcListener - The listener function to
	 * 	check for.
	 * 	.
	 * @return {boolean} - Returns TRUE if the listener function is
	 * 	currently subscribed to the player's timeupdate event.
	 * 	Otherwise it returns FALSE.
	 */
	this.isListenerSubscribed = function(funcListener) {
		let errPrefix = '(isListenerSubscribed) ';

		if (typeof funcListener !== 'function')
			throw new Error(errPrefix + `The value in the funcListener parameter is not a function.`);

        return self.aryTimeUpdateListeners.includes(funcListener);
    };

	/**
     * Subscribe to the YouTube player's timeupdate event.
     *
     * @param {function} funcListener - The listener function to
	 * 	be called when the player's timeupdate event is triggered.
     */
	this.addTimeUpdateListener = function(funcListener) {
		let errPrefix = '(addTimeUpdateListener) ';

		if (typeof funcListener !== 'function')
			throw new Error(errPrefix + `The value in the funcListener parameter is not a function.`);

		// If this listener already exists in the array, do nothing.
        if (!self.isListenerSubscribed(funcListener))
			// New subscriber, add them.
            this.aryTimeUpdateListeners.push(funcListener);
    };

	/**
	 * Unsubscribe from the YouTube player's timeupdate event.
	 *
	 * @param {function} funcListener - The listener function to
	 * 	remove from the subscriber list.
	 */
	this.removeTimeUpdateListener = function(funcListener) {
		let errPrefix = '(removeTimeUpdateListener) ';

		if (typeof funcListener !== 'function')
			throw new Error(errPrefix + `The value in the funcListener parameter is not a function.`);

		// If this listener already does not exist in the array, do nothing.
		if (self.isListenerSubscribed(funcListener))
			// New subscriber, add them.
			this.aryTimeUpdateListeners.push(funcListener);
	};
	...
	this.isVideoDataAvailable = function()
	{
		...
		if (misc_shared_lib_yas.isEmptySafeString(self.youtubePlayer._player.getVideoData().title))
			return false;
	...
	/**
	 * This function is called when the video player's timeupdate
	 *  event is triggered.  If any listeners are subscribed to
	 *  that event, they will be notified with the current
	 *  time location of the video.
	 *
	 * @param {Number} currentSeconds - The current video location
	 *  in seconds from start.
	 */
	this.notifyTimeUpdateListeners = function(currentSeconds)
    {
        // Notify all time update listeners.
        for (var i = 0; i < self.aryTimeUpdateListeners.length; i++)
        {
            self.aryTimeUpdateListeners[i](currentSeconds);
        }
    }
	...
	this.initializeYouTubePlayerAPI = function()
	{
		...
		self.youtubePlayer.on(
			...
			(currentSeconds) =>
			{
				let strTime = misc_shared_lib_yas.secondsToHoursMinutesSecondsString(currentSeconds);
				...
				// Notify any listeners that the video location has changed.
				self.notifyTimeUpdateListeners(currentSeconds);
			});
	}
	...
	{
		let errPrefix = '(getCurrentVideoTitle) ';
		...
		return new Promise(
			function(resolve, reject)
			{
				...
				if (misc_shared_lib_yas.isEmptySafeString(videoId))
					reject(errPrefix + 'The video ID is empty.');
				
				let bIsPlaybackRequired = false;
				...
				else
				{
					...
					let videoTitle = self.youtubePlayer._player.getVideoData().title;
					bIsPlaybackRequired = misc_shared_lib_yas.isEmptySafeString(videoTitle);
				}
...
if (typeof module == 'undefined' || typeof module.exports == 'undefined')
{
	...
	// Use the browser instance of misc_shared_lib.
	misc_shared_lib_yas = misc_shared_lib;
}
else {
	// Server context.
	common_routines = require("../../../common/common-routines");
	youtube_search = require('youtube-api-v3-search');
	YouTubeSimpleApiClass = require('simple-youtube-api');

	// Get our YouTube API key from the environment.
	let youTubeApiKey = common_routines.getEnvironmentVarOrError('YOUTUBE_API_KEY');

	youtubeSimpleApi = new YouTubeSimpleApiClass(youTubeApiKey);
	misc_shared_lib_yas = require ('../misc/misc-shared');
